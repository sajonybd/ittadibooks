...
export default function HomepageVideos() {
  ...
  const fetchVideos = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/videos/homepage`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editVideo) {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/videos/homepage/edit/${editVideo._id}`, {
          method: "PUT",
        ...
        // Add new video
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/videos/homepage`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData),
        });
...
const handleDelete = async (id) => {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/videos/homepage/edit/${id}`, { method: "DELETE" });
    const data = await res.json();
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Homepage Video Management</h1>
        <button
          onClick={() => handleOpen()}
