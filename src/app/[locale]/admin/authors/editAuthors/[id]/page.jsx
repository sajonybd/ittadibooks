...
export default function EditAuthorPage() {
  ...
  useEffect(() => {
    const fetchAuthor = async () => {
      try {
        ...
        // prefill date pickers if dob/dod exist
        if (data.dob) {
          const [day, month, year] = data.dob.split("/");
          setDobDate(new Date(`${year}-${month}-${day}`));
        }
        if (data.dod) {
          const [day, month, year] = data.dod.split("/");
          setDodDate(new Date(`${year}-${month}-${day}`));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/authors/update/${id}`,
        {
          method: "PUT",
          body: data,
        }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-md mt-2">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Edit Author</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Name fields */}
        <div className="flex gap-4">
          <div className="w-full">
            <label className="block mb-1 font-medium">Author Name (English)</label>
            <input
              type="text"
              name="name"
              value={formData.name}
          ...
          <div className="w-full">
            <label className="block mb-1 font-medium">Author Name (বাংলা)</label>
            <input
              type="text"
              name="nameBn"
          ...
          <div className="w-full">
            ...
            <DatePicker
              ...
              onChange={(date) => {
                ...
                if (date) {
                  const formatted = new Intl.DateTimeFormat("en-GB").format(date);
                  setFormData((prev) => ({ ...prev, dod: formatted }));
                } else {
        ...
        <div>
          ...
          {(formData.previewImage || formData.existingImage) && (
            <div className="mb-2 flex items-center gap-4">
              <img
                src={formData.previewImage || formData.existingImage}
                alt="Preview"
